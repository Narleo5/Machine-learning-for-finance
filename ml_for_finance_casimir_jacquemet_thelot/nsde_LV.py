    ...
    for epoch in range(n_epochs):
        ...
        for i in range(0, 20*batch_size, batch_size):
            ...
            if requires_grad_CV:
                ...
                loss = MSE
                loss_train_temp.append(np.sqrt(loss.item())) # RMSE
                init_time = time.time()
                loss.backward()
        ...
        if loss_train_temp: # if not empty
            loss_train.append(sum(loss_train_temp)/len(loss_train_temp))
        ...
        MSE = loss_fn(pred, target_mat_T)
        loss_val=torch.sqrt(MSE) # RMSE
        loss_test.append(loss_val.item())
        print('epoch={}/{}, loss={:.4f}'.format(epoch, n_epochs, loss_val.item()))
        
        # save checkpooint
        if loss_val < loss_val_best:
             model_best = model
             loss_val_best=loss_val
             print('loss_val_best', loss_val_best.item())
...
def plot_loss(test, train):
    ind_test = range(len(test))
    ind_train = [ind_test[i] for i in range(len(ind_test)) if i % 2 == 0]
    fig, ax = plt.subplots()
    plt.plot(ind_test, test, label='test')
    plt.plot(ind_train, train, label='train')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--n_layers', type=int, default=4) # 4 hidden layers
    parser.add_argument('--vNetWidth', type=int, default=50) # 50 neurons each
    ## pour sigma, sigma^S, sigma^V et b^V
    ...
    ## Load market prices and set training target (1 or 2)
    
    ## 1 - data from the article
    # data = torch.load("Call_prices_59.pt")
    # data = data[:6,:6]
    
    ## 2 - market prices (SPX or SX5E)
    data = pd.read_csv('target_SPX_price.csv', index_col=0).values

    # Set up training - Strike values, time discretisation and maturities
    strikes_call = np.linspace(1., 1.25, 6)
    n_steps = 96
    timegrid = torch.linspace(0, 1, n_steps+1).to(device) 
    maturities = [16, 32, 48, 64, 70, 96] # for [60, 120, 180, 240, 300, 360] or [30, 60, 120, 150, 180]
    # maturities = [m//16 for m in maturities]
    n_maturities = len(maturities)
    ...
    ## Modifier ici le nombre d'epochs
    ...
    ## Pour un charger un modèle déjà entrainé (SX5E : market ou call59 : article)
    # model = Net_LV(dim=1, timegrid=timegrid, strikes_call=strikes_call, n_layers=args.n_layers, vNetWidth=args.vNetWidth, device=device, n_maturities=n_maturities, maturities=maturities, rate=rate)
    # model.load_state_dict(torch.load('pretrained/SX5E_MC10k_batch2k_epochs2k.pth'))
    # model.eval()
    ...
    surfaces = model(S0=1, z=z_test, MC_samples=MC_samples_test, ind_T=n_steps, period_length=16)
    
    ## plot des preds et de la var, avec et sans la même échelle
    for i in range(len(titles)):
        
        surface = surfaces[i]
        X, Y = log_moneyness, np.array(maturities) / (96*2) # ou strikes_call, np.array(maturities) # *2 si market data car on va jusqu'au milieu de l'année seulement
        X, Y = np.meshgrid(X, Y)
        Z = surface.detach().numpy()
    ...
    plot_loss(loss_test, loss_train)
    
    y_pred = surfaces[0].detach().numpy()
    print('MAE : {}'.format(abs(y_pred-data).mean()))
    
    ## pour sauvegarder le modele pytorch
    # torch.save(model.state_dict(), 'path/modelname.pth')
